...
from app.models.schemas import Position, SortableColumn, SortOrder, AggregationType
...
def test_data_service_init(mock_scan_parquet, sample_df):
    ...
    service = DataService()
    assert service._df.shape == sample_df.shape
    assert service._metadata["total_drafts"] == 2
    assert service._metadata["total_teams"] == 2
    assert service._metadata["total_players"] == 4  # QB1, RB1, WR1, RB2

@patch("app.services.data_service.pl.scan_parquet")
@pytest.mark.parametrize("positions, search_term, expected_count", [
    (None, None, 4),  # All unique players
    ([Position.QB], None, 1),
    (None, "QB", 1),
])
def test_get_players(mock_scan_parquet, sample_df, positions, search_term, expected_count):
    mock_scan_parquet.return_value = sample_df.lazy()
    service = DataService()
    players, total = service.get_players(positions=positions, search_term=search_term, limit=100, offset=0)
    assert total == expected_count
    assert len(players) == min(expected_count, 100)
...
def test_get_player_details(mock_scan_parquet, sample_df):
    ...
    assert details["avg_pick"] == 2.5  # (1+4)/2
    assert details["total_drafts"] == 2
...
def test_get_position_stats(mock_scan_parquet, sample_df):
    ...
    assert qb_stats.median_draft_count == 1.0  # 1 QB per draft
...
def test_get_first_player_draft_stats(mock_scan_parquet, sample_df):
    ...
    qb_stats = next(s for s in stats if s['Position'] == 'QB')
    assert qb_stats['avg_first_pick'] == 2.5
    assert qb_stats['min_first_pick'] == 1
    assert qb_stats['max_first_pick'] == 4

@patch("app.services.data_service.pl.scan_parquet")
def test_get_position_draft_counts_by_round(mock_scan_parquet, sample_df):
    mock_scan_parquet.return_value = sample_df.lazy()
    service = DataService()
    counts = service.get_position_draft_counts_by_round(Position.QB, AggregationType.MEAN)
    assert len(counts) == 2
    assert counts[0].round == 1
    assert counts[0].count == 0.5
    assert counts[1].round == 2
    assert counts[1].count == 0.5
...
def test_get_player_combinations(mock_scan_parquet, sample_df):
    mock_scan_parquet.return_value = sample_df.lazy()
    service = DataService()
    combos = service.get_player_combinations(required_players=["QB1", "WR1"], n_rounds=2, limit=10)
    assert len(combos) > 0  # Based on sample, teams with both
