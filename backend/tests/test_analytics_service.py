...
def test_get_players(mock_duckdb_service, mock_data_service):
    ...
    df_players = pl.DataFrame([{"player": "Test", "Position": "QB", "Team": "TM", "avg_pick": 10.0, "min_pick": 5, "max_pick": 15, "draft_percentage": 50.0}])
    mock_duckdb_service.query.side_effect = [df_total_drafts, df_total_count, df_players]
...
def test_get_players_fallback(mock_duckdb_service, mock_data_service):
    mock_duckdb_service.query.side_effect = lambda sql: pl.DataFrame({"n": [100]}) if 'COUNT(DISTINCT draft)' in sql else pl.DataFrame({"cnt": [0]}) if 'COUNT(*)' in sql else pl.DataFrame()
    with patch('time.perf_counter', side_effect=[0, 0.06, 0.061, 0.07]):  # dur_duck=0.06, dur_pol=0.009 < 0.048
