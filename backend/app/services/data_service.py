    ...
    class DataService:
        ...
        def _get_data_path(self) -> str:
            ...
            data_path = os.path.join(
                backend_dir, "..", "data", "updated_bestball_data.parquet"
            )
        ...
        def _initialize_data(self) -> None:
            ...
            logger.info(
                f"Data initialization complete: {len(all_players)} players, {total_drafts} drafts, {total_teams} teams"
            )
        ...
        def get_players(
            ...
            if positions:
                ...
                player_stats_query = player_stats_query.filter(
                    pl.col("Position").is_in(position_values)
                )
            ...
            if search_term:
                ...
                player_stats_query = player_stats_query.filter(
                    ...
                    .str.to_lowercase()
                    .str.replace_all(r"[\.\']", "")
                    .str.contains(normalized_search_term)
                )
            ...
            final_query = player_stats_query.sort(
                sort_by.value, descending=(sort_order == SortOrder.DESC)
            ).slice(offset, limit)
        ...
        def get_position_draft_counts_by_round(
            ...
            counts_per_draft = position_df.group_by(["round", "draft"]).agg(
                pl.len().alias("count")
            )
        ...
        def get_player_combinations(
            ...
            logger.info(
                f"Finding unique combinations for {n_rounds} rounds with {len(required_players)} required players"
            )
            ...
            result_df = (
                df.lazy()
                .filter(
                    pl.col("team_id").is_in(relevant_team_ids)
                    & (pl.col("round") <= n_rounds)
                )
                .sort("team_id", "round")
                .group_by("team_id")
                .agg(
            ...
            if not position_counts_collected_df.is_empty():
                pos_cols = sorted(
                    [
                        col
                        for col in position_counts_collected_df.columns
                        if col != "team_id"
                    ]
                )
                format_exprs = [
                    pl.format("{}: {}", pl.lit(col), pl.col(col)) for col in pos_cols
                ]

                position_counts_str_df = (
                    position_counts_collected_df.lazy()
                    .with_columns(
                        pl.concat_str(format_exprs, separator=", ").alias(
                            "position_counts"
                        )
                    )
