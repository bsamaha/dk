...
async def get_players(
    positions: Optional[List[Position]] = Query(
        None, description="Filter by positions"
    ),
    search_term: Optional[str] = Query(
        None, description="Search term for player names"
    ),
    limit: int = Query(100, description="Maximum number of results", ge=1, le=1000),
    offset: int = Query(0, description="Offset for pagination", ge=0),
    sort_by: Optional[SortableColumn] = Query(
    ...
    try:
        logger.info(
            f"Fetching players with filters: positions={positions}, search_term={search_term}"
        )
        ...
        return PlayersResponse(
            players=players, page_info=page_info_obj, total_count=total_count
        )
...
async def search_players(
    ...
    try:
        ...
        players, total_count = analytics_service.get_players(
            search_term=q, limit=limit, offset=0
        )

        return {"query": q, "results": players, "total_found": total_count}
