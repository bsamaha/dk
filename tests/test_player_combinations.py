...
def test_no_required_players(sample_data):
    ...
    result = find_unique_player_combinations(
        sample_data, n_rounds=2, required_players=[]
    )
    expected_data = {
...
def test_with_required_players(sample_data):
    ...
    result = find_unique_player_combinations(
        sample_data, n_rounds=2, required_players=["PlayerA"]
    )
    expected_data = {
        "team_id": [101, 102],
        "draft": [1, 1],
...
def test_multiple_required_players(sample_data):
    ...
    result = find_unique_player_combinations(
        sample_data, n_rounds=2, required_players=["PlayerA", "PlayerC"]
    )
    expected_data = {
        "team_id": [102],
        "draft": [1],
...
def test_n_rounds_filter(sample_data):
    ...
    result = find_unique_player_combinations(
        sample_data, n_rounds=1, required_players=["PlayerA"]
    )
    expected_data = {
        "team_id": [102],
        "draft": [1],
...
def test_no_matching_players(sample_data):
    ...
    result = find_unique_player_combinations(
        sample_data, n_rounds=2, required_players=["PlayerZ"]
    )
...
def test_empty_dataframe():
    ...
    empty_df = pl.DataFrame(
        ...
        schema={
            ...
            "round": pl.Int64,
            "team_id": pl.Int64,
            "draft_position": pl.Int64,
            "player": pl.Utf8,
        },
    )
    result = find_unique_player_combinations(
        empty_df, n_rounds=2, required_players=["PlayerA"]
    )
    assert result.is_empty()
