...
*Routers remain thin (no heavy logic) and delegate to the unified `QueryService` via DI (`Depends(get_query_service)`). Blocking service calls are offloaded with `run_in_threadpool` directly.*
...
3. Backend `analytics_router.get_draft_slot_correlation` delegates to `QueryService.get_draft_slot_correlation(slot, metric, top_n)` (DuckDB).
...
| Caching | Browser (TanStack) + backend lightweight in-process cache |
...
1. **Add new endpoint**
   1. Add a method in `services/query_service.py` (parameterized SQL; whitelist dynamic parts).
   2. Expose via a new function in appropriate `api/*.py` using `Depends(get_query_service)` and `run_in_threadpool`.
   3. Update `models/schemas.py` only if a new response shape is needed.
...
| `app/services/query_service.py` | All queries via DuckDB + Polars |
| `app/models/schemas.py` | Pydantic models (core enums & responses) |
...
### 6.3 QueryService Highlights

| Function | Description (Polars) |
|----------|----------------------|
| `get_players` | Filter + sort via SQL, paginated |
| `get_player_details` | Aggregates via SQL |
| `get_player_combinations` | Teams that drafted required players within N rounds |
| `get_roster_construction` | Pivot counts per position, aggregate counts |

**Note**: Lightweight in-process cache for a few hot reads; no external cache.

### 6.4 DuckDB

DuckDB is embedded and leveraged for SQL-heavy aggregations (heat map, stack finder, ADP drift, draft slot correlation). Query paths are optimized for DuckDB, with Polars used for light shaping where helpful.

Key points:

- Single in-memory DuckDB connection, `PRAGMA enable_object_cache`, view on Parquet file.
- Small materialized tables (e.g., `heat_map_cached`) for hot reads.
